...
const contextMenuOption = $ref([
  {
    ...
    icon: renderIcon(ICON.F, 'mdi:refresh')
  },
  {
    label: '关闭',
    key: 'close',
    // disabled: tagsStore.tags.length <= 1,
    icon: renderIcon(ICON.F, 'mdi:close')
  },
  {
    label: '关闭其他',
    key: 'close-other',
    // disabled: tagsStore.tags.length <= 1,
    icon: renderIcon(ICON.F, 'mdi:close-box-outline')
  },
  {
    label: '关闭左侧',
    key: 'close-left',
    // disabled: tagsStore.tags.length <= 1 || props.currentPath === tagsStore.tags[0].path,
    icon: renderIcon(ICON.F, 'mdi:format-pilcrow-arrow-left')
  },
  {
    label: '关闭右侧',
    key: 'close-right',
    // disabled: tagsStore.tags.length <= 1 || props.currentPath === tagsStore.tags[tagsStore.tags.length - 1].path,
    icon: renderIcon(ICON.F, 'mdi:format-pilcrow-arrow-right')
  }
]);
